...
let turnOn (targets:List<CoOrd>) =
    List.map (fun (target:CoOrd) -> lights.[target.x, target.y] <- lights.[target.x, target.y] + 1) targets

let turnOff (targets:List<CoOrd>) =
    targets |>
    List.map (fun (target:CoOrd) -> match lights.[target.x, target.y] with
                                    | 0 -> ()
                                    | _ -> lights.[target.x, target.y] <- lights.[target.x, target.y] - 1)

let toggle (targets:List<CoOrd>) =
    List.map (fun (target:CoOrd) -> lights.[target.x, target.y] <- lights.[target.x, target.y] + 2) targets
...
lights |> Seq.cast<int> |> Seq.reduce (+)